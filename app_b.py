...
def build_segments(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        open_dt, final_dt = r.get("opening_date"), r.get("deadline")
        first_dt, second_dt = r.get("first_deadline"), r.get("second_deadline")
        ...
        if two_stage:
            if pd.notna(open_dt) and pd.notna(first_dt) and open_dt <= first_dt:
                rows.append({"y_label": y_label, "programme": prog,
                             "start": open_dt, "end": first_dt,
                             "segment": "Stage 1", "title": title,
...
def build_altair_chart_from_segments(seg: pd.DataFrame, view_start, view_end):
    ...
    chart_height = max(560, len(y_order) * 50)
    domain_min, domain_max = pd.to_datetime(view_start), pd.to_datetime(view_end)
    min_x, max_x = seg["start"].min(), seg["end"].max()

    bands_df = build_month_bands(min_x, max_x)
    month_shade = alt.Chart(bands_df).mark_rect().encode(
        x="start:T", x2="end:T",
        opacity=alt.Opacity("band:Q", scale=alt.Scale(domain=[0,1], range=[0.0, 0.08]), legend=None),
        color=alt.value("#000")
    )
    months = pd.date_range(min_x.to_period("M").start_time, max_x.to_period("M").end_time, freq="MS")
    weeks = pd.date_range(min_x.to_period("W-MON").start_time, max_x.to_period("W-MON").start_time, freq="W-MON")
...
raw = pd.read_excel(xls, sheet_name=st.selectbox("Sheet", xls.sheet_names, index=0))
df = canonicalise(raw)
...
trl_opts     = sorted([str(int(x)) for x in df.get("trl", pd.Series(dtype=float)).dropna().unique()])
...
with st.form("filters_form", clear_on_submit=False):
    st.header("Filters")
    c1,c2 = st.columns(2)
    programmes = c1.multiselect("Programme", options=prog_opts, default=prog_opts)
    clusters   = c2.multiselect("Cluster", options=cluster_opts)
    c3,c4 = st.columns(2)
    types = c3.multiselect("Type of Action", options=type_opts)
    trls  = c4.multiselect("TRL", options=trl_opts)
    dests = st.multiselect("Destination / Strand", options=dest_opts)

    st.subheader("Search (multi-keyword)")
    c5,c6,c7 = st.columns(3)
    kw1, kw2, kw3 = c5.text_input("Keyword 1"), c6.text_input("Keyword 2"), c7.text_input("Keyword 3")
    ...
    dead_all = pd.concat([pd.to_datetime(df.get("deadline"), errors="coerce"),
    ...
    c8,c9 = st.columns(2)
    open_start = c8.date_input("Open from", value=open_lo, min_value=open_lo, max_value=open_hi)
    open_end   = c9.date_input("Open to",   value=open_hi, min_value=open_lo, max_value=open_hi)
    c10,c11 = st.columns(2)
    close_from = c10.date_input("Close from", value=dead_lo, min_value=dead_lo, max_value=dead_hi)
    close_to   = c11.date_input("Close to",   value=dead_hi, min_value=dead_lo, max_value=dead_hi)

    bud_series = pd.to_numeric(df.get("budget_per_project_eur"), errors="coerce").dropna()
    min_bud, max_bud = (0.0, 1_000_000.0) if bud_series.empty else (float(bud_series.min()), float(bud_series.max()))
    if not (min_bud < max_bud): min_bud, max_bud = max(min_bud,0.0), min_bud+100000.0
    budget_range = st.slider("Budget per project (EUR)", min_bud, max_bud,
                             (min_bud,max_bud), step=100000.0)
...
if applied:
    st.session_state.criteria = dict(programmes=programmes, clusters=clusters, types=types, trls=trls, dests=dests,
...
if not st.session_state.criteria:
    st.session_state.criteria = dict(programmes=prog_opts, clusters=[], types=[], trls=[], dests=[],
...
f = f[(f["opening_date"] >= pd.to_datetime(crit["open_start"])) & (f["opening_date"] <= pd.to_datetime(crit["open_end"]))]
any_end_in = (
    (pd.to_datetime(f.get("deadline"), errors="coerce").between(crit["close_from"], crit["close_to"], inclusive="both")) |
    (pd.to_datetime(f.get("first_deadline"), errors="coerce").between(crit["close_from"], crit["close_to"], inclusive="both")) |
    (pd.to_datetime(f.get("second_deadline"), errors="coerce").between(crit["close_from"], crit["close_to"], inclusive="both"))
)
...
with tab1:
    ...
    if not f.empty:
        data_min = min(pd.to_datetime(f["opening_date"].min(), errors="coerce"),
                       pd.to_datetime(f["deadline"].min(), errors="coerce"))
        data_max = max(pd.to_datetime(f["opening_date"].max(), errors="coerce"),
                       pd.to_datetime(f["deadline"].max(), errors="coerce"))
        pad = pd.Timedelta(days=30)
        view_start, view_end = (data_min - pad).date(), (data_max + pad).date()
    else:
        view_start, view_end = open_lo, open_hi
    chart = build_altair_chart_from_segments(segments, view_start, view_end)
    st.altair_chart(chart, use_container_width=False) if chart is not None else st.info("No rows with valid dates to display.")
...
with tab3:
    ...
    for _, row in f.iterrows():
        ...
        with st.expander(title):
            c1, c2 = st.columns(2)
            with c1:
                if pd.notna(row.get("opening_date")): st.markdown(f"📅 **Opening:** {row.get('opening_date'):%d %b %Y}")
                if pd.notna(row.get("deadline")):     st.markdown(f"⏳ **Deadline:** {row.get('deadline'):%d %b %Y}")
                if row.get("two_stage"):
                    if pd.notna(row.get("first_deadline")):  st.markdown(f"🔄 **Stage 1:** {row.get('first_deadline'):%d %b %Y}")
                    if pd.notna(row.get("second_deadline")): st.markdown(f"🔄 **Stage 2:** {row.get('second_deadline'):%d %b %Y}")
            with c2:
                if pd.notna(row.get("budget_per_project_eur")): st.markdown(f"💶 **Budget per project:** {row.get('budget_per_project_eur'):,.0f} EUR")
                if pd.notna(row.get("total_budget_eur")):       st.markdown(f"📦 **Total budget:** {row.get('total_budget_eur'):,.0f} EUR")
                if pd.notna(row.get("num_projects")):           st.markdown(f"📊 **# Projects:** {int(row.get('num_projects'))}")
            st.markdown(f"🏷️ **Programme:** {row.get('programme','-')}  | **Cluster:** {row.get('cluster','-')}  | "
                        f"**Destination:** {row.get('destination_or_strand','-')}  | **Type of Action:** {row.get('type_of_action','-')}  | **TRL:** {row.get('trl','-')}")
            if row.get("expected_outcome"):
                with st.expander("🎯 Expected Outcome"):
                    st.markdown(highlight_text(row.get("expected_outcome"), kw_list), unsafe_allow_html=True)
            if row.get("scope"):
                with st.expander("🧭 Scope"):
                    st.markdown(highlight_text(row.get("scope"), kw_list), unsafe_allow_html=True)
            if row.get("full_text"):
                with st.expander("📖 Full Description"):
                    st.markdown(highlight_text(row.get("full_text"), kw_list), unsafe_allow_html=True)
            st.caption(f"📂 Source: {row.get('source_filename','-')} | Version: {row.get('version_label','-')} | Parsed on: {row.get('parsed_on_utc','-')}")
