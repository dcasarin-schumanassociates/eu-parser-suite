...
def build_altair_chart_from_segments(seg: pd.DataFrame, view_start, view_end):
    ...
    month_labels = alt.Chart(month_labels_df).mark_text(
        ...
        text="label:N",
        y=alt.value(0)  # fixed position at top
    )
...
with tab3:
    ...
    for _, row in f.iterrows():
        ...
        # --- New Tab4: Gantt with Persistent Axis ---
tab4, = st.tabs(["🪄 Gantt (Persistent Axis)"])

with tab4:
    st.subheader("Gantt with Persistent X-Axis (Experimental)")
    segments = build_segments(f)
    if segments.empty:
        st.info("No rows with valid dates to display.")
    else:
        ...
        # --- Axis-only chart (top row)
        x_axis = alt.Chart(month_labels_df).mark_text(
            align="center",
            baseline="top",
            dy=5,
            fontSize=12,
        ...
        # --- Bars chart (bottom row)
        y_order = seg["y_label"].drop_duplicates().tolist()
        row_height = 50
        chart_height = max(700, len(y_order) * row_height)

        bars = alt.Chart(seg).mark_bar(cornerRadius=3, color="#1E90FF").encode(
            y=alt.Y(
                "y_label:N",
                sort=y_order,
                axis=alt.Axis(title=None, labelFontSize=11, labelLimit=500),
        ...
        # --- Stack vertically, share x-scale
        chart = alt.vconcat(x_axis, bars).resolve_scale(x="shared")
        ...
        st.markdown('<div class="scroll-container-axis">', unsafe_allow_html=True)
        st.altair_chart(chart, use_container_width=False)
        st.markdown('</div>', unsafe_allow_html=True)
